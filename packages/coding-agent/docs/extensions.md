...
> **Placement for /reload:** Put extensions in `~/.trek/agent/extensions/` (global) or `.pi/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.trek/agent/extensions/my-extension.ts`:

```typescript
import type { ExtensionAPI } from "@trek/coding-agent";
...
| Location | Scope |
|----------|-------|
| `~/.trek/agent/extensions/*.ts` | Global (all projects) |
| `~/.trek/agent/extensions/*/index.ts` | Global (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@trek/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@trek/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@trek/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@trek/coding-agent";
...
```typescript
import { isBashToolResult } from "@trek/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@trek/coding-agent";
...
```typescript
import { SessionManager } from "@trek/coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@trek/ai";
...
```typescript
import type { AutocompleteItem } from "@trek/tui";
...
```typescript
import { withFileMutationQueue } from "@trek/coding-agent";
...
**Important:** Use `StringEnum` from `@trek/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@trek/coding-agent";
...
```typescript
import { createBashTool } from "@trek/coding-agent";
...
```typescript
import { keyHint } from "@trek/coding-agent";
...
```typescript
import { Text, Component } from "@trek/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@trek/coding-agent";
import { matchesKey } from "@trek/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@trek/coding-agent";
