...
Content`,
		...
		it("should auto-discover root markdown skills from .trek skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .trek", async () => {
			const extDir = join(tempDir, ".trek", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".trek"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".trek", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".trek", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".trek", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".trek", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".trek", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .trek/agent skills", async () => {
		...
		it("should use the project .trek dir as baseDir for project .trek skills", async () => {
			const projectBaseDir = join(tempDir, ".trek");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".trek", "agent");
		...
		it("should dedupe user skill entries when ~/.trek/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .trek auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".trek\n");

			const skillDir = join(tempDir, ".trek", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".trek", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .trek settings base", () => {
			...
			const rel = relative(join(tempDir, ".trek"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".trek", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".trek", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".trek", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".trek", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".trek", "npm", "node_modules", "example");
