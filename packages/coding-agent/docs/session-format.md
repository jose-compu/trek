...
```
~/.trek/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.trek/agent/sessions/`.
...
For TypeScript definitions in your project, inspect `node_modules/@trek/coding-agent/dist/` and `node_modules/@trek/ai/dist/`.
...
### Extended Message Types (from trek-coding-agent)
...
Each O→I→A→R cycle is stored as `customType: "trek:reflective_cycle"`. Same loop as 0.3.0; schema v1 adds timestamps, cycle id, tool names, and laws pre/post. Inspect with `trek trace list`, `trek trace show <cycleId>`, `trek trace diff <cycleId>`, and `trek trace fork <cycleId>`. After Reflect, a distinct `trek:cycle_checkpoint` entry is appended (not `trek:edit_batch_checkpoint`). Module: `packages/coding-agent/src/core/trace/`.
