...
describe("package commands", () => {
	...
	let originalTrekLatestVersionUrl: string | undefined;
	...
	beforeEach(() => {
		...
		originalTrekLatestVersionUrl = process.env.TREK_LATEST_VERSION_URL;
		process.env.TREK_LATEST_VERSION_URL = "https://example.test/latest";
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("trek install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "trek --help" or "trek install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: trek install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "trek-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		mkdirSync(join(projectDir, ".trek"), { recursive: true });
		writeFileSync(
			fakeNpmPath,
...
`,
		...
		writeFileSync(
			join(projectDir, ".trek", "settings.json"),
	...
	it("uses the current package name when the update check omits packageName", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@mariozechner", "trek-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
