...
function createTempDir(): string {
	const dir = mkdtempSync(join(tmpdir(), "trek-session-id-readonly-"));
	tempDirs.push(dir);
	return dir;
}
...
function writeSession(sessionDir: string, cwd: string, id: string): void {
	...
	writeFileSync(
		...
		`${JSON.stringify({ type: "session", version: 3, id, timestamp: new Date().toISOString(), cwd: resolvedCwd })}\n`,
	);
}
...
describe("--session-id read-only commands", () => {
	...
	// TODO(0.2.0): fork duplicate-session guard is not reached before -p auth in subprocess CLI path.
	it.skip("rejects an existing fork target session id", async () => {
