...
```typescript
import { createAgentSession, SessionManager } from "@trek/coding-agent";
...
The `Agent` class (from `@trek/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.trek/agent", // default (expands ~)
});
```
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.trek/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@trek/coding-agent";
...
// Default: uses ~/.trek/agent/auth.json and ~/.trek/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@trek/coding-agent";
...
```typescript
import { createAgentSession } from "@trek/coding-agent";
...
import { createAgentSession, defineTool } from "@trek/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.trek/agent/extensions/`, `.pi/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@trek/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@trek/coding-agent";
...
1. Global: `~/.trek/agent/settings.json`
2. Project: `<cwd>/.trek/settings.json`
