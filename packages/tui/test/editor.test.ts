...
describe("Editor component", () => {
	...
	describe("borderLabel", () => {
		it("renders a fixed label on the top border row", () => {
			const editor = new Editor(createTestTUI(), defaultEditorTheme, { borderLabel: "[#3]" });
			...
			assert.ok(contentLine?.trim().includes("hello"));
			assert.ok(!contentLine?.includes("[#3]"));
		});
	});
});
