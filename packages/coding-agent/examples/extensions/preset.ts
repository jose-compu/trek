 ...
 * - ~/.trek/agent/presets.json (global)
 * - <cwd>/.trek/presets.json (project-local)
 *
...
import type { Api, Model } from "@trek/ai";
import type { ExtensionAPI, ExtensionContext } from "@trek/coding-agent";
import { DynamicBorder, getAgentDir } from "@trek/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@trek/tui";
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.trek/agent/presets.json or .pi/presets.json", "warning");
			return;
		}
