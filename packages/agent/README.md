...
Stateful agent with tool execution and event streaming. Built on `@trek/ai`.

## Installation

```bash
npm install @trek/agent-core
```
...
```typescript
declare module "@trek/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@trek/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@trek/agent-core";
