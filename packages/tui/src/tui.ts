...
export class TUI extends Container {
	...
	private showHardwareCursor = isTrekEnvTruthy("HARDWARE_CURSOR");
	private clearOnShrink = isTrekEnvTruthy("CLEAR_ON_SHRINK"); // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = isTrekEnvTruthy("DEBUG_REDRAW");
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".pi", "agent", "trek-debug.log");
		...
		// Configurable via setClearOnShrink() or TREK_CLEAR_ON_SHRINK=0 env var
