...
async function testContextOverflow(
	model: Model<any>,
	apiKey: string,
	options?: { omitSystemPrompt?: boolean },
): Promise<OverflowResult> {
	const overflowContent = generateOverflowContent(model.contextWindow);

	const context: Context = {
		...(options?.omitSystemPrompt ? {} : { systemPrompt: "You are a helpful assistant." }),
		messages: [
			{
				role: "user",
...
// LM Studio (local) — probe for a loadable model before tests run
let lmStudioRunning = false;
let lmStudioModelId: string | undefined;

async function detectLmStudio(): Promise<void> {
	if (process.env.PI_NO_LOCAL_LLM) {
		return;
	}

	try {
		const ping = await fetch("http://localhost:1234/v1/models", { signal: AbortSignal.timeout(1000) });
		if (!ping.ok) {
			return;
		}
	} catch {
	...
	const candidates = [process.env.LM_STUDIO_MODEL, "mistral-7b-instruct-v0.2", "liquid/lfm2.5-1.2b"].filter(
...
describe("Context overflow error handling", () => {
	...
	// =============================================================================
	// LM Studio (local) - Skip if not running, no loadable model, or local LLM tests disabled
	// =============================================================================

	describe.skipIf(!lmStudioRunning)("LM Studio (local)", () => {
		it("should detect overflow via isContextOverflow", async () => {
			const model: Model<"openai-completions"> = {
				id: lmStudioModelId ?? "local-model",
				api: "openai-completions",
				provider: "lm-studio",
			...
			const result = await testContextOverflow(model, "lm-studio", { omitSystemPrompt: true });
			logResult(result);
