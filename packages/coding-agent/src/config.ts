...
import { isTrekEnvTruthy, trekEnv } from "./utils/trek-env.ts";
...
export function getPackageDir(): string {
	...
	const envDir = process.env.TREK_PACKAGE_DIR ?? trekEnv("PACKAGE_DIR");
	if (envDir) {
...
// =============================================================================
// App Config (from package.json trekConfig, piConfig fallback for Pi plugins)
// =============================================================================
...
const appConfig = pkg.trekConfig ?? pkg.piConfig;
const trekConfigName: string | undefined = appConfig?.name;
export const PACKAGE_NAME: string = pkg.name || "@trek/coding-agent";
export const APP_NAME: string = trekConfigName || "trek";
export const APP_TITLE: string = APP_NAME;
export const CONFIG_DIR_NAME: string = appConfig?.configDir || ".trek";
export const VERSION: string = pkg.version || "0.0.0";

// e.g., TREK_CODING_AGENT_DIR (legacy: PI_CODING_AGENT_DIR)
export const ENV_AGENT_DIR = `${APP_NAME.toUpperCase()}_CODING_AGENT_DIR`;
export const ENV_SESSION_DIR = `${APP_NAME.toUpperCase()}_CODING_AGENT_SESSION_DIR`;

export { isTrekEnvTruthy, trekEnv };
...
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = trekEnv("SHARE_VIEWER_URL") || DEFAULT_SHARE_VIEWER_URL;
	return `${baseUrl}#${gistId}`;
}

// =============================================================================
// User Config Paths (~/.trek/agent/*)
// =============================================================================

/** Get the agent config directory (e.g., ~/.trek/agent/) */
export function getAgentDir(): string {
	const envDir = process.env[ENV_AGENT_DIR] ?? trekEnv("CODING_AGENT_DIR");
	if (envDir) {
...
/** Get path to TUI render snapshot dump (Shift+Ctrl+D / /debug) */
export function getRenderDebugLogPath(): string {
	return join(getAgentDir(), `${APP_NAME}-render-debug.log`);
}

/** Global constitution / prologue path (~/.trek/prologue.md) */
export function getGlobalProloguePath(): string {
	return join(homedir(), CONFIG_DIR_NAME, "prologue.md");
}

/** Project-local prologue override (.trek/prologue.md) */
export function getProjectProloguePath(cwd: string): string {
	return join(cwd, CONFIG_DIR_NAME, "prologue.md");
}
