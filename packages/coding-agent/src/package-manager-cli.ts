import { Markdown, type MarkdownTheme } from "@trek/tui";
...
Options:
  -l, --local    Install project-locally (.trek/settings.json)
...
Options:
  -l, --local    Remove from project settings (.trek/settings.json)
...
Short forms:
  ...
  ${APP_NAME} update pi             Update ${APP_NAME} only (pi is a legacy alias)
`);
			return;
