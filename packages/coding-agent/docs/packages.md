...
By default, `install` and `remove` write to user settings (`~/.trek/agent/settings.json`). Use `-l` to write to project settings (`.trek/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup.
...
- User installs go under `~/.trek/agent/npm/`.
...
- Cloned to `~/.trek/agent/git/<host>/<path>` (global) or `.pi/git/<host>/<path>` (project).
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@trek/ai`, `@trek/agent-core`, `@trek/coding-agent`, `@trek/tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.trek/agent`) and project (`.pi/`) scopes.
