...
import type { ThinkingLevel } from "@trek/agent-core";
import chalk from "chalk";
import { APP_NAME, CONFIG_DIR_NAME, ENV_AGENT_DIR, ENV_SESSION_DIR, getDebugLogPath } from "../config.ts";
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--help" || arg === "-h") {
			...
			if (output === "text" || output === "jsonl") {
				...
				result.diagnostics.push({
					type: "error",
					message: `--output must be "text" or "jsonl", got "${output}"`,
				});
			}
		} else if (arg === "--offline") {
			result.offline = true;
...
${chalk.bold("Options:")}
  ...
  --offline                      Disable startup network operations (same as TREK_OFFLINE=1)
...
${chalk.bold("Environment Variables:")}
  ...
  TREK_PACKAGE_DIR                 - Override package directory (for Nix/Guix store paths)
  TREK_OFFLINE                     - Disable startup network operations when set to 1/true/yes
  TREK_TELEMETRY                  - Override install telemetry when set to 1/true/yes or 0/false/no
  ...
  TREK_NO_LOCAL_LLM                - Skip local LLM integration tests
  TREK_CACHE_RETENTION             - Prompt cache retention: "long" or default short
  TREK_HARDWARE_CURSOR             - Show hardware cursor in TUI
  TREK_CLEAR_ON_SHRINK             - Clear empty TUI rows when content shrinks

  Legacy PI_* names are still accepted for the variables above.
