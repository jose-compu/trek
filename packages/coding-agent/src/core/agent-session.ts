...
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@trek/ai";
import {
	clampThinkingLevel,
...
import {
	createReflectiveMetaToolDefinitions,
...
export type AgentSessionEvent =
	...
	| { type: "prompt_meta"; promptNumber: number; promptId: string; preview?: string };
...
export class AgentSession {
	...
	private async _emitExtensionEvent(event: AgentEvent): Promise<void> {
		if (event.type === "agent_start") {
			...
			await this._extensionRunner.emit(extensionEvent);
			debugLog("tool", "execution start", { toolName: event.toolName, toolCallId: event.toolCallId });
	...
	private _rebuildSystemPrompt(toolNames: string[]): string {
		...
		this._loadedPrologue = loadPrologue(this._cwd);
		...
		this._baseSystemPromptOptions = {
			...
			appendSystemPrompt,
			prologue: this._loadedPrologue.content,
	...
	private _ensureSystemPromptTrace(): void {
		if (this.sessionManager.hasSystemPromptTrace()) {
			return;
		}

		const trace = {
			systemPromptHash: hashContent(this._baseSystemPrompt),
			prologueHash: this._loadedPrologue.content ? hashContent(this._loadedPrologue.content) : undefined,
			prologueSources: this._loadedPrologue.sources.length > 0 ? this._loadedPrologue.sources : undefined,
		};
		this.sessionManager.appendSystemPromptTrace(trace);
	...
	private async _runAgentPrompt(messages: AgentMessage | AgentMessage[]): Promise<void> {
		try {
			...
			while (await this._handlePostAgentRun()) {
				if (!(await this._safeAgentContinue())) {
					break;
				}
			}
		} finally {
	...
	/**
	 * Drop completed assistant turns at the end of context so continue/retry can run
	 * after compaction (context often ends on a kept assistant message).
	 */
	private _trimTrailingAssistantsForRetry(): void {
		const messages = this.agent.state.messages;
		let end = messages.length;
		while (end > 0 && messages[end - 1].role === "assistant") {
			const assistant = messages[end - 1] as AssistantMessage;
	...
	/** Continue the agent loop when context ends on user/toolResult; handle queued steer/follow-up. */
	private async _safeAgentContinue(): Promise<boolean> {
		...
		if (last.role === "assistant") {
			...
			this._trimTrailingAssistantsForRetry();
		}

		const lastAfter = this.agent.state.messages[this.agent.state.messages.length - 1];
		if (!lastAfter || lastAfter.role === "assistant") {
			return false;
		}
	...
	private _buildRuntime(options: {
		...
		if (this._reflectiveLoop) {
			for (const definition of createReflectiveMetaToolDefinitions(
				this._reflectiveLoop,
				this._cwd,
				() => this.agent.state.messages.length,
				() => this._lastPromptTask,
			)) {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		if (/repetitive thinking loop/i.test(err)) return false;
