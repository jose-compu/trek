...
import { CONFIG_DIR_NAME, isTrekEnvTruthy } from "../config.ts";
...
function isOfflineModeEnabled(): boolean {
	return isTrekEnvTruthy("OFFLINE");
}
...
type SkillDiscoveryMode = "pi" | "trek" | "agents";
...
function readPiManifestFile(packageJsonPath: string): PiManifest | null {
	try {
		...
		const pkg = JSON.parse(content) as { pi?: PiManifest; trek?: PiManifest };
		return pkg.trek ?? pkg.pi ?? null;
	} catch {
		return null;
	}
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "trek");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// @trek/* peers. Stale auto-installed pi peers can otherwise block updates.
	...
	private addAutoDiscoveredResources(
		...
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "trek"),
			projectMetadata,
		...
		// User extensions from ~/.trek/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.trek/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "trek"),
			userMetadata,
