...
import { type AssistantMessage, getModel } from "@trek/ai";
...
describe("AgentSession auto-compaction queue resume", () => {
	...
	it("should ignore stale pre-compaction assistant usage on pre-prompt compaction checks", async () => {
		...
		const staleAssistant: AssistantMessage = {
			...
			model: model.id,
			usage: {
				input: Math.max(model.contextWindow - 1_000, 1),
				output: 10_000,
				cacheRead: 0,
				cacheWrite: 0,
				totalTokens: Math.max(model.contextWindow - 1_000, 1),
				cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 },
			},
			stopReason: "stop",
	...
	it("should trim trailing assistant messages before overflow retry continue", async () => {
		const model = session.model!;
		const userMessage = {
			role: "user" as const,
		...
		session.agent.state.messages = [userMessage, keptAssistant];

		const safeContinue = (
			session as unknown as {
				_safeAgentContinue: () => Promise<boolean>;
			}
		)._safeAgentContinue.bind(session);
		const continueSpy = vi.spyOn(session.agent, "continue").mockResolvedValue();

		await expect(safeContinue()).resolves.toBe(true);
