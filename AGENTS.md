...
- Pre-commit blocks lockfile commits unless `TREK_ALLOW_LOCKFILE_CHANGE=1` (legacy: `PI_ALLOW_LOCKFILE_CHANGE=1`). Don't bypass unless the user wants the lockfile change committed.
...
tmux send-keys -t pi-test "./trek-test.sh" Enter
...
3. **Run the release script**:
   ```bash
   TREK_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:patch    # fixes + additions
   TREK_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:minor    # breaking changes
   ```
