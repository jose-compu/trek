...
export interface EditorOptions {
	...
	/** Label drawn on the top border (e.g. `[#3]` + dashes). Not inline with typed text. */
	borderLabel?: string;
}
...
export class Editor implements Component, Focusable {
	...
	constructor(tui: TUI, theme: EditorTheme, options: EditorOptions = {}) {
		...
		this.borderLabel = options.borderLabel ?? "";
	}
	...
	setBorderLabel(label: string): void {
		const newLabel = label ?? "";
		if (this.borderLabel !== newLabel) {
			this.borderLabel = newLabel;
			this.tui.requestRender();
		}
	}
	...
	render(width: number): string[] {
		...
		// Render top border (with scroll indicator if scrolled down, or border label when at top)
		if (this.scrollOffset > 0) {
			...
			const labelWidth = visibleWidth(this.borderLabel);
			const dashes = horizontal.repeat(Math.max(0, width - labelWidth));
			result.push(this.borderColor(this.borderLabel + dashes));
		} else {
