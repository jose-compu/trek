...
describe("parseArgs", () => {
	...
	describe("prologue CLI flags", () => {
		test("parses --debug", () => {
			expect(parseArgs(["--debug"]).debug).toBe(true);
		});

		test("parses --batch with file", () => {
			expect(parseArgs(["--batch", "prompts.txt"]).batch).toBe("prompts.txt");
		});

		test("parses --batch without file as stdin", () => {
			expect(parseArgs(["--batch"]).batch).toBe(true);
		});

		test("parses --output jsonl", () => {
			expect(parseArgs(["--output", "jsonl"]).output).toBe("jsonl");
		});
	});
});
