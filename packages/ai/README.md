...
TypeBox exports are re-exported from `@trek/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@trek/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@trek/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@trek/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@trek/ai';
...
```typescript
import { getImageModel, generateImages } from '@trek/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@trek/ai';
...
```typescript
import { getModel, stream } from '@trek/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@trek/ai';
...
```typescript
import { Model, stream } from '@trek/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@trek/ai';
...
```typescript
import { getModel, complete, Context } from '@trek/ai';
...
```typescript
import { Context, getModel, complete } from '@trek/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@trek/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@trek/ai';
...
```bash
npx @trek/ai login              # interactive provider selection
npx @trek/ai login anthropic    # login to specific provider
npx @trek/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@trek/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from '@trek/ai/oauth';
...
```typescript
import { getModel, complete } from '@trek/ai';
import { getOAuthApiKey } from '@trek/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@trek/ai`
